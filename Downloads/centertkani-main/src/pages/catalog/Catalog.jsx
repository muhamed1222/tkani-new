...
export const Catalog = observer(() => {
  ...
  // Автоматическое построение breadcrumbs
  ...
  return (
    <div className="flex flex-col gap-[10px] items-center px-0 py-[20px] w-full min-h-screen bg-[#F1F0EE]">
        ...
        {/* Заголовок категории */}
        <div className="bg-[rgba(155,30,28,0.1)] flex items-center justify-start lg:justify-center px-[14px] md:px-[16px] lg:px-[20px] py-[14px] w-full relative">
          ...
          {/* Кнопка категорий с выпадающим меню - только на мобилке и планшете */}
          <div className="absolute right-[14px] md:right-[16px] lg:hidden" ref={categoriesRef}>
            <button
              onClick={() => setIsCategoriesOpen(!isCategoriesOpen)}
              className="flex items-center gap-[6px] font-inter font-medium leading-[1.2] text-[#9b1e1c] text-[16px] hover:opacity-80 transition-opacity"
            >
              <span>Категории</span>
              <svg
                width="16"
            ...
            {isCategoriesOpen && (
              <div className={`absolute right-0 top-full mt-[8px] bg-white border border-[rgba(16,16,16,0.1)] rounded-[8px] shadow-lg z-50 min-w-[200px] max-h-[400px] overflow-y-auto ${styles.categoriesDropdown}`}>
        ...
        <div className={`flex flex-col gap-[16px] items-start w-full max-w-[1440px] px-[14px] md:px-[16px] lg:px-[20px] ${styles.containerPadding}`}>
          ...
          <div className={`flex items-start py-0 w-full ${styles.catalogLayout}`}>
            {/* Боковая панель навигации - показывается только на десктопе */}
            <div className={`hidden lg:flex flex-col gap-[5px] items-start shrink-0 ${styles.sidebar}`}>
              {fabricTypes.map((type) => {
                if (!shouldShowFilter(type)) return null;
