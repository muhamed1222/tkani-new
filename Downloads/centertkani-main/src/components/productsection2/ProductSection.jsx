...
export const ProductSection = memo(({ title, products = [], keyPrefix = "", noPadding = false }) => {
  ...
  const containerClasses = noPadding 
    ? "max-w-[1440px] w-full mx-auto py-[22px] md:py-[34px] lg:py-[46px]" 
    : "max-w-[1440px] w-full mx-auto px-[14px] md:px-[16px] lg:px-[20px] py-[22px] md:py-[34px] lg:py-[46px]";
      ...
      <div className="flex flex-col gap-[16px]">
        ...
        <div className={styles.productGrid}>
          {visibleProducts.map((product, index) => {
            ...
            return (
              <ProductCard 
                key={keyPrefix ? `${keyPrefix}-${product.id || index}` : product.id || index} 
                product={product} 
                showHover={true} 
              />
            );
  ...
  return (
    ...
    prevProps.products.every((product, index) => {
      ...
      return product && nextProduct && (product.id === nextProduct.id || 
        (product.name === nextProduct.name && product.title === nextProduct.title));
    })
  );
});