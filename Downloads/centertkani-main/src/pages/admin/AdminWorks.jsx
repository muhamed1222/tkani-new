...
export const AdminWorks = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      logger.error("Ошибка сохранения работы:", err);
    }
  };
  ...
  const handleDelete = async (workId) => {
    ...
    try {
      ...
      logger.error("Ошибка удаления работы:", err);
    }
  };
  ...
  const getImageUrl = (imagePath) => {
    ...
    return `${strapiUrl}/static/works/${imagePath}`;
  };
            ...
            {works.map((work) => (
              <tr key={work.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {(() => {
                    // Используем первое изображение из массива images, или fallback на image
