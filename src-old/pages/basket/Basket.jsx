                ...
                const getImageUrl = () => {
                  if (product.images && product.images.length > 0) {
                    ...
                    if (image.attributes && image.attributes.url) {
                      return `http://localhost:1338${image.attributes.url}`;
                    }
                    if (image.url) {
                      return image.url.startsWith('http') ? image.url : `http://localhost:1338${image.url}`;
                    }
                    if (image.formats) {
                    ...
                    if (typeof image === 'string') {
                      return image.startsWith('http') ? image : `http://localhost:1338${image}`;
                    }
                  }
                  
                  if (product.image) {
                    return product.image.startsWith('http') ? product.image : `http://localhost:1338${product.image}`;
                  }
                  if (product.img) {
                    return product.img.startsWith('http') ? product.img : `http://localhost:1338${product.img}`;
                  }
