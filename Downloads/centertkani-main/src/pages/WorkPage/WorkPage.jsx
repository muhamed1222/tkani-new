...
export const WorkPage = observer(() => {
  ...
  // Получаем изображения: сначала массив images, если пустой - используем image как fallback
  let images = work.images || [];
  
  // Если массив images пустой, но есть поле image - используем его
  if (images.length === 0 && work.image) {
    images = [work.image];
  }
  
  // Фильтруем валидные изображения (исключаем те, на которых была ошибка)
  ...
  logger.log('🖼️ WorkPage images debug:', {
    workId: work.id,
    workTitle: work.title,
    hasImagesArray: !!work.images,
    imagesArrayLength: work.images?.length || 0,
    imagesArray: work.images,
    hasImageField: !!work.image,
    imageField: work.image,
  ...
  const handleAddToCart = async (productId, e) => {
    ...
    try {
      const response = await cartAPI.addToCart(productId, 1.0); // Добавляем 1 метр по умолчанию
      
      // Проверяем, был ли товар уже в корзине
      const wasExisting = response.data?.wasExisting || false;
      
      if (wasExisting) {
        showToast('Метраж товара увеличен на 1 м', 'success');
      } else {
  ...
  // Используем связанные ткани из работы, если они есть, иначе показываем пустой массив
  const featuredProducts = work.fabrics && work.fabrics.length > 0 
    ? work.fabrics.slice(0, 3) 
    : [];
