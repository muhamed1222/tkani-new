...
import { getAvatarUrl as getAvatarUrlUtil } from "../../utils/apiConfig"; 
...
export const Personal_account = observer(() => {
  ...
  // Функция для получения URL аватара (мемоизирована)
  const getAvatarUrl = useCallback(() => {
    ...
    if (user.user.avatar) {
      const avatarUrl = getAvatarUrlUtil(user.user.avatar);
      if (avatarUrl) {
        return avatarUrl;
      }
    }
    ...
    return DEFAULT_AVATAR_IMAGE; // статичный fallback аватар
  }, [user.user]);
  
  // Мемоизированный URL аватара
  ...
  useEffect(() => {
    const checkAuthentication = async () => {
      try {
        logger.log('🔐 Personal_account - проверка авторизации...');
        
        // Если пользователь не загружен, проверяем авторизацию
        if (!user.user || Object.keys(user.user).length === 0) {
          logger.log('🔄 Personal_account - пользователь не загружен, проверяем auth...');
          await user.checkAuth();
        }
        
        // Если после проверки пользователь все еще не авторизован, редирект
        if (!user.isAuth) {
          logger.log('❌ Personal_account - пользователь не авторизован, редирект...');
          navigate(LOGIN_ROUTE);
          return;
        }
        
        logger.log('✅ Personal_account - пользователь авторизован:', user.user);
        
      } catch (error) {
        logger.error('❌ Personal_account - ошибка проверки авторизации:', error);
        navigate(LOGIN_ROUTE);
      } finally {
    ...
    // Проверяем только если пользователь не авторизован или не загружен
    if (!user.isAuth || !user.user || Object.keys(user.user).length === 0) {
      checkAuthentication();
    } else {
      setIsPageLoading(false);
    }
  ...
  useEffect(() => {
    if (user.user && user.isAuth) {
      logger.log('🔄 Personal_account - синхронизация данных пользователя');
      
      // Проверяем структуру объекта пользователя
      logger.log('👤 Структура объекта user.user:', JSON.stringify(user.user, null, 2));
      
      // Пробуем получить данные из разных возможных полей
      const firstNameValue = user.user.firstName || user.user.first_name || user.user.firstname || "";
      const lastNameValue = user.user.lastName || user.user.last_name || user.user.lastname || "";
      const middleNameValue = user.user.middleName || user.user.middle_name || "";
      const emailValue = user.user.email || "";
      const phoneValue = user.user.phone || user.user.phoneNumber || "";
      
      logger.log('📝 Извлеченные данные:', {
      ...
      setOriginalMiddleName(middleNameValue);
  ...
  useEffect(() => {
    const hasChanged = firstName !== originalFirstName || lastName !== originalLastName || middleName !== originalMiddleName;
    setIsPersonalDataChanged(hasChanged);
    if (hasChanged) {
      setPersonalDataError("");
      setPersonalDataSuccess("");
    }
  }, [firstName, lastName, middleName, originalFirstName, originalLastName, originalMiddleName]);
  ...
  // Очистка ошибок при изменении полей (объединено в один useEffect для оптимизации)
  useEffect(() => {
    if (firstName) setFirstNameError("");
    if (lastName) setLastNameError("");
    if (middleName) setMiddleNameError("");
    if (email) setEmailError("");
    if (phone) setPhoneError("");
    if (newPassword) setNewPasswordError("");
    if (confirmPassword) setConfirmPasswordError("");
  }, [firstName, lastName, middleName, email, phone, newPassword, confirmPassword]);

  // Отслеживание изменений пароля
  useEffect(() => {
    const hasChanged = !!(oldPassword || newPassword || confirmPassword);
    setIsPasswordChanged(hasChanged);
    if (hasChanged) {
      setPasswordError("");
      setPasswordSuccess("");
    }
  ...
  const validatePhone = (value) => {
    ...
    // Проверяем российский формат: начинается с 7 или 8, содержит 11 цифр
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      logger.error('Ошибка загрузки аватара (catch):', {
        message: error.message,
        status: error.status,
        stack: error.stack
      });
      setAvatarError(error.message || "Неожиданная ошибка при загрузке аватара");
      setTimeout(() => {
        setAvatarError("");
      }, 5000);
    } finally {
  ...
  const handleSavePersonalData = async (e) => {
    e.preventDefault();
    logger.log('🔵 handleSavePersonalData - начало', { firstName, lastName, middleName });
    setPersonalDataError("");
    setPersonalDataSuccess("");
    ...
    try {
      logger.log('🔵 handleSavePersonalData - вызов user.updateProfile с данными:', { 
      ...
      logger.log('🟢 handleSavePersonalData - полный результат:', JSON.stringify(result, null, 2));
      ...
      if (result.success) {
        ...
        logger.error('❌ handleSavePersonalData - ошибка:', result.error);
      ...
      logger.error('🔴 handleSavePersonalData - исключение:', error);
  ...
  const handleSaveContactData = async (e) => {
    e.preventDefault();
    logger.log('🔵 handleSaveContactData - начало', { email, phone });
    setContactDataError("");
    setContactDataSuccess("");
    setEmailError("");
    setEmailSuccess("");
    ...
    try {
      logger.log('🔵 handleSaveContactData - вызов user.updateProfile с данными:', { 
        email: email.trim(),
        phone: phone.trim()
      });
      ...
      logger.log('🟢 handleSaveContactData - полный результат:', JSON.stringify(result, null, 2));
      ...
      if (result.success) {
        ...
        logger.error('❌ handleSaveContactData - ошибка:', result.error);
      ...
      logger.error('🔴 handleSaveContactData - исключение:', error);
      logger.error('🔴 Детали ошибки:', JSON.stringify(error, null, 2));
  ...
  const handleChangePassword = async (e) => {
    e.preventDefault();
    logger.log('🔐 handleChangePassword - начало смены пароля');
    setPasswordError("");
    setPasswordSuccess("");
    setNewPasswordError("");
    ...
    try {
      logger.log('🔐 Отправляем запрос на смену пароля...');
      const result = await user.changePassword(oldPassword, newPassword);
      
      logger.log('🔐 Результат смены пароля:', JSON.stringify(result, null, 2));
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      logger.log('🗑️ Начало удаления аккаунта - используем прямой метод');
      
      const result = await user.deleteAccountDirect();
      
      logger.log('🗑️ Результат удаления:', result);
          ...
          <div className={styles.name_fields}>
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                className={`${styles.input} ${middleNameError ? styles.inputError : ""}`}
              ...
              />
              {middleNameError && (
                <p id="middlename-error" className={styles.fieldError} role="alert">
                  {middleNameError}
                </p>
              )}
            </div>
