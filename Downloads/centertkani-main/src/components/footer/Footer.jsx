...
export const Footer = observer(() => {
  ...
  const clothingCategoriesFromStrapi = catalogSections.clothing.length > 0
    ? catalogSections.clothing
    : clothingCategories;
  
  const homeCategoriesFromStrapi = catalogSections.home.length > 0
    ? catalogSections.home
    : homeCategories;
        ...
        <div className={styles.footer_content}>
          ...
          <div className={styles.footer_section}>
            ...
            <nav className={styles.nav} aria-label="Категории для одежды">
              {clothingCategoriesFromStrapi.map((category, index) => {
                const categorySlug = category.slug;
                const categoryName = category.name;
                const isActive = isClothingCatalog && isActiveCategory(categorySlug);
                return (
                  <Link 
                    key={index}
                    to={`${CATALOG_CLOTHING_ROUTE}/${categorySlug}`} 
          ...
          <div className={styles.footer_section}>
            ...
            <nav className={styles.nav} aria-label="Категории для дома">
              {homeCategoriesFromStrapi.map((category, index) => {
                const categorySlug = category.slug;
                const categoryName = category.name;
                const isActive = !isClothingCatalog && isActiveCategory(categorySlug);
                return (
