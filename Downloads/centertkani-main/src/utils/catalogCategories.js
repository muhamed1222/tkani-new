...
// Определение типа каталога (clothing/home) по slug категории
export const getCategoryCatalogType = (categorySlug) => {
  if (!categorySlug) return null;
  
  // Проверяем, есть ли категория в списке для одежды
  const isClothing = clothingCategories.some(cat => cat.slug === categorySlug);
  if (isClothing) return 'clothing';
  
  // Проверяем, есть ли категория в списке для дома
  const isHome = homeCategories.some(cat => cat.slug === categorySlug);
